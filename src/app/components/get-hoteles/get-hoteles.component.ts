...
export class GetHotelesComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild('paginator', { static: false }) paginator: MatPaginator | null =
    null;
  ...
  public onSubmit() {
    this.serviceHoteles
      .getFiltrosHoteles(
        this.filtroForm.controls['nombreHotel'].value,
        this.filtroForm.controls['codigoHotel'].value,
        this.filtroForm.controls['ciudadHotel'].value
      )
      .subscribe(
        (data) => {
  ...
  eliminar(elemento: Hoteles) {
    ...
    dialogoRef.afterClosed().subscribe((respuesta) => {
      console.log(respuesta);
      if (respuesta) {
        this.serviceHoteles.eliminar(elemento.idHotel).subscribe((data) => {
